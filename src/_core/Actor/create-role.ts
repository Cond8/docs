...
export type NeedsActorsScript<T extends CoreRedprint> = (actorScript: ActorScript<T>) => StagedActor<T>;

export type NeedsActorsScriptAndMetadata<T extends CoreRedprint> = (actorScript: ActorScript<T>) => NeedsMetadata<T>;

export function createRole<T extends CoreRedprint>(actorScript: ActorScript<T>): NeedsMetadata<T>;

export function createRole<T extends CoreRedprint>(actorName: string, ...metadata: unknown[]): NeedsActorsScript<T>;

export function createRole<T extends CoreRedprint>(metaHook: MetaHook, ...metadata: unknown[]): NeedsActorsScriptAndMetadata<T>;

export function createRole<T extends CoreRedprint>(firstArg: string | MetaHook | ActorScript<T>, ...restArgs: unknown[]) {
