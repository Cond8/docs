...
const links = [
  ...
  { url: '/scenes/actors', title: 'Actors', rightSeparator: true },
  { url: '/docs', title: 'Docs' },
...
export const Link = ({ idx, isSelected }: LinkProps) => {
  ...
  return (
    <>
      ...
      {rightSeparator ? (
        <div
          className="hidden md:block h-4 w-[2px] bg-muted-foreground"
          aria-hidden="true"
        />
      ) : null}
    </>
...
export const TopbarLinks = ({ urlSelected }: TopbarLinksProps) => (
  <nav
    className="flex flex-col md:flex-row items-center font-bold space-y-2 md:space-y-0 md:space-x-4"
